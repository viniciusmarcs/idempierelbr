...
public class NFeXMLGenerator {
	...
	public static String geraCorpoNFe (Properties ctx, int LBR_NotaFiscal_ID, String trxName) throws Exception {
		...
		for (MLBRNotaFiscalLine nfLine : nfLines) {
			...
			MLBRDocLineDetails details = MLBRDocLineDetails.getOfPO(nfLine);
			...
			if(productEAN == null || !NFeUtil.isValidEAN(productEAN.trim()))
				productEAN = "SEM GTIN";
			...
			// CEST
			MLBRDocLineICMS[] icmsLines = MLBRDocLineICMS.getOfDetails(MLBRDocLineDetails.getOfPO(nfLine));
			...
			produtos.setcEANTrib(productEAN);
